...
def load_dataset(dataset:str):
    """
    Return a DataFrame with examples from dataset
    """
...
if __name__ == '__main__':
    ...
    # oracle.load_state_dict(torch.load(oracle_state_dict_path, weights_only=True))
    # oracle_samples = torch.load(oracle_samples_path, weights_only=True).type(torch.LongTensor)
    ...
    if CUDA:
        ...
        gen = gen.cuda()
        dis = dis.cuda()
        oracle_samples = helpers.batchwise_sample(oracle, 10_000, BATCH_SIZE)
        oracle_samples = oracle_samples.cuda()

    # print(oracle_samples);print(oracle_samples.shape)
    # exit()
